...
#
# MQTT agent to look at mobiles registered in the freebox
# and broadcast the mac addresses and signal on mqtt queue
#
#
...
while True:
	...
	for (max,hostname,signal) in result:
	    hostname = string.replace(hostname,"+","_")
	    # print hostname
	    client2.publish(WIFI_ROOT + "/" + hostname, str(signal))
